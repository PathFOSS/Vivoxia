...
public class FoodJournal extends Fragment implements DateChangeListener, EntryAddedListener {
    ...
    public FoodJournal (ViewChangeListener viewChangeListener, TopBarListener topBarListener) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.fragment_food_journal, container, false);
        ...
        view.findViewById(R.id.ib_add).setOnClickListener(v -> new FoodSelector(topBarListener, this).show(getParentFragmentManager(), "Food Selector"));
    ...
    private void drawEntries(){
        ...
        for (FoodJournalEntry foodJournalEntry : foodJournalDataBase.getEntriesForDate(measurementDate)) {

            ConstraintLayout child = (ConstraintLayout) getLayoutInflater().inflate(R.layout.journal_item, viewGroup, false);
            child.setId(foodJournalEntry.getRowID());

            String topLeftText = foodJournalEntry.getName();
            String topRightText = Units.decimalFormat.format(foodJournalEntry.getCalories()) + " kcal";
            String bottomLeftText = Units.decimalFormat.format(foodJournalEntry.getAmount()) + " " + foodJournalEntry.getMassUnit();
            String bottomRightText = "P " + Units.decimalFormat.format(foodJournalEntry.getProtein()) + " g  |  F " + Units.decimalFormat.format(foodJournalEntry.getFat()) + " g  |  C " + Units.decimalFormat.format(foodJournalEntry.getCarbs()) + " g";
            ...
            createFoodEntryViewListener(child);
        }
    }
    ...
    private void setSelectedListeners (ConstraintLayout constraintLayout) {
        ...
        ibDuplicate.setOnClickListener( v -> {
            foodJournalDataBase.duplicateEntry(foodSelectedID);
        ...
        ibDelete.setOnClickListener( v -> {
            foodJournalDataBase.deleteEntry(foodSelectedID);
    ...
    private void createFoodEntryViewListener(@NonNull ConstraintLayout foodItemLayout){
        foodItemLayout.setOnClickListener(v -> {
            if (!foodSelected) {
                toggleButtons(true);
                foodSelectedID = foodItemLayout.getId();
                ReusableMethods.colorJournalTextViews(foodItemLayout.findViewById(R.id.tv_top_left), foodItemLayout.findViewById(R.id.tv_top_right), Controller.getColorVivoxia());
                setSelectedListeners(foodItemLayout);
            } else if (foodSelectedID == foodItemLayout.getId()) {
                Toast.makeText(getContext(), "Canceled", Toast.LENGTH_SHORT).show();
                toggleButtons(false);
                ReusableMethods.colorJournalTextViews(foodItemLayout.findViewById(R.id.tv_top_left), foodItemLayout.findViewById(R.id.tv_top_right), Controller.getColorNaturalWhite());
            } else if (foodSelected) {
                Toast.makeText(getContext(), "Swapped entries", Toast.LENGTH_SHORT).show();
                foodJournalDataBase.swapEntries(foodSelectedID, foodItemLayout.getId());
                toggleButtons(false);
                ReusableMethods.colorJournalTextViews(foodItemLayout.findViewById(R.id.tv_top_left), foodItemLayout.findViewById(R.id.tv_top_right), Controller.getColorNaturalWhite());
                drawEntries();
            }
